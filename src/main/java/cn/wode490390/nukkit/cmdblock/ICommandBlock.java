...
import cn.wode490390.nukkit.cmdblock.scheduler.CommandBlockTrigger;
...
public interface ICommandBlock extends CommandSender, InventoryHolder {
    ...
    default boolean trigger(int chain) {
        ...
        if (delay > 0) {
            Server.getInstance().getScheduler().scheduleDelayedTask(CommandBlockPlugin.getInstance(), new CommandBlockTrigger(this, chain), delay);
            return false;
        }
